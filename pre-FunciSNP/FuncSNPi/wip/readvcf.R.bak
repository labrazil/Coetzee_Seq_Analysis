...
FunciSNP <- function(ethno = c("AFR", "ASN", "EUR", "AMR", "ALL"), 
                     bio.features.loc, snp.regions.file, output.file = risk.snp, R.squared.cutoff = 0) {
    cat("
####################################
##                                ##
##      Welcome to FunciSNP       ##
...
"\n ### and new ones will be placed there, press
 ...
 ### 10 second countdown:", "\n")
    ...
    cat("testing 1000 genomes server \n")
    cat("testing ebi \n")
    server.up <- try(url(paste(ebi, manifest.file, sep=""), open = "rt"), silent = TRUE)
    server.error <- inherits(server.up, "try-error")
    if(server.error) {
        cat("ebi failed \ntrying ebi \n")
        rm(server.error)
        server.up <- try(url(paste(ncbi, manifest.file, sep=""), open = "rt"), silent = TRUE)
        server.error <- inherits(server.up, "try-error")
        if(server.error) {
            cat("ncbi failed \n")
            stop("Neither EBI nor NCBI mirrors for the 1000 genomes project were found")
        } else {
            cat("OK using ncbi: ", ncbi, "\n")
        }
    } else {
        cat("OK using ebi :", ebi, " \n")
    }
    ...
    for(j in as.character(snp.names)) {
        ...
        cat(paste("Pulling In Variants File for SNP: ", j, "\n", sep=""))
        for(i in as.character(ethno)) {
            ...
            for(h in bio.features.file) {
                ...
                cat("began searching for overlap with biofeature: \n")
                ...
                snp.geno <- FilterByFeatures(h, variants.data)
                snp.ld.frame <- LDTesting(snp.geno, snp.ld.frame, bio.features.count, snp.region, variants.data)
...
PullInVariants <- function(ethno.chosen, snp.names.chosen, snp.regions) {
    ...
    snp.names.chosen <<- as.character(snp.names.chosen)
...
FilterByFeatures <- function(features.file, variants.data) {
    close.snp.ranges <- 
...
LDTesting <- function(snps, snp.ld.frame = snp.ld.frame, bf.count, snp.regions, var.data) {
