...
#		cut -f1,2 plinkformat.${input}.$riskname.$ethnic.$region.bed > plinkformat.${input}.$riskname.$ethnic.$region.bed.pos
#		grep $riskname ${input}.$riskname.$ethnic.$region.vcf | cut -f1,2 >> plinkformat.${input}.$riskname.$ethnic.$region.bed.pos
#		sed -i s/chr// plinkformat.${input}.$riskname.$ethnic.$region.bed.pos
#		sed -i '1ichromosome \t position' plinkformat.${input}.$riskname.$ethnic.$region.bed.pos
...
#		mv plinkformat.ped $DIR/plinkformat.${input}.$riskname.$ethnic.$region.$(basename ${file%.bed}).ped
    java -jar /usr/local/bin/Haploview.jar -nogui -log haploview_results/${input}.$riskname.$ethnic.$region.hap.log -out haploview_results/${input}.$riskname.$ethnic.$region -pedfile plinkformat.${input}.$riskname.$ethnic.$region.ped -info plinkformat.${input}.$riskname.$ethnic.$region.markers -skipcheck -dprime -memory 1500
