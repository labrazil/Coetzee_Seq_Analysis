...
FunciSNP <- function(ethno = c("AFR", "ASN", "EUR", "AMR", "ALL"), 
                     bio.features.loc, snp.regions.file, output.file = "risk.snp", R.squared.cutoff = 0, primary.server, makePlots = TRUE) {
    cat("
        ####################################
        ##                                ##
        ##      Welcome to FunciSNP       ##
        ...
        try(dir.create(path="FunciSNP_plots", showWarnings=FALSE), silent=TRUE) ## directory used to store plots
        try(dir.create(path="FunciSNP_tmp", showWarnings=FALSE), silent=TRUE) ## directory used to store temp files
        try(dir.create(path="FunciSNP_log", showWarnings=FALSE),silent=TRUE) ## directory used to store log files
        try(dir.create(path="FunciSNP_tables", showWarnings=FALSE),silent=TRUE) ## directory used to store summary tables
        ...
        if(makePlots == TRUE) {
                ...
                cat("##########################################\nFunciSNP is complete!\nSee 'FunciSNP_tables' and 'FunciSNP_plots' folder for overall results\nSee", output.file, "for complete results.\n##########################################")
        } else {
            FunciSNPSummary(R.2=R.squared.cutoff, dat = snp.ld.frame)
            cat("##########################################\nFunciSNP is complete!\nSee 'FunciSNP_tables' folder for overall results\nSee", output.file, "for complete results.\n##########################################")
        }
}
...
FunciSNPPlot <- function(R.2, dat) {
    ...
    for( i in 1:length(summary(as.factor(all[,"feature"]))) ){
        ...
        facet_wrap(~ snp.names.chosen)
        ggsave(file=paste("FunciSNP_plots/",bio[i],"_R2vsDist_riskSNP.pdf",sep=""))
        cat("Finished plotting ", i, "/",length(bio), "\n")
    }
    cat("\n\nMaking plots is finished, see 'plots' folder in ", getwd(), "\n\n")
}
FunciSNPHeatmap <- function(R.2, dat) {
    ...
    dev.off()
    cat("\n\nHeatmap plot is complete, see 'FunciSNP_plots' folder in ", getwd(),"\n\n")
}
